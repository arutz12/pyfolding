...
fun! pyfolding#default(name, default) "{{{
    if !exists(a:name)
...
let g:pyfolding_regex = '^\s*\%(class\|def\|async\s\+def\) .\+\(:\s\+\w\)\@!'
let s:def_regex = g:pyfolding_regex
let s:blank_regex = '^\s*$'
...
fun! pyfolding#expr(lnum) "{{{
    ...
    " g:pyfolding_nest_limit lines due to performance concerns
    if line('$') < g:pyfolding_nest_limit && indent(prevnonblank(a:lnum))
        let curpos = getpos('.')
        try
